...
async function handleCreateRace() {
	try{
		if(store.player_id && store.track_id){
			...
			//hide accelerator button aka gas pedal
			...
			// if user fails to select a track or racer, alert them!
			alert('Please select a racer and track to start.')
		}
	} catch (err){
		console.log(err)
	}
...
async function getTracks() {
	try{
		let trackInfo  = await fetch(`${SERVER}/api/tracks`);// GET request to `${SERVER}/api/tracks`
		return trackInfo.json();
	}
	catch(err){
		console.log("Problem with getTracks response::", err);
	}
}

async function getRacers() {
	try{
		const racersInfo = await fetch(`${SERVER}/api/cars`);// GET request to `${SERVER}/api/cars`
		return racersInfo.json();
	}
	catch(err){
		console.log("Problem with getRacers response::", err);
	}
}
...
async function getRace(id) {
	try{
		let raceInfo = await fetch(`${SERVER}/api/races/${id}`); // GET request to `${SERVER}/api/races/${id}`
		return raceInfo.json();
	}
	catch(err){
		console.log("Problem with getRace response::", err)
	}
}
